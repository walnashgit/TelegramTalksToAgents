...
DRIVE_FOLDER_ID = os.getenv('DRIVE_FOLDER_ID', '')  # Working directory in Google Drive
...
mcp = FastMCP(
    ...
    instructions=None,
    auth_server_provider=None,  # or some real provider
    dependencies=["google-auth", "google-auth-oauthlib", "google-api-python-client"],
    lifespan=spreadsheet_lifespan,
    host="127.0.0.1",
    port=7171
...
@mcp.tool()
def add_data_to_spreadsheet(spreadsheet_id: str,
                sheet: str,
                range: str,
                data: List[List[Any]],
                ctx: Context = None) -> Dict[str, Any]:
    ...
    Returns:
        Result of the update operation
    """
    return update_cells(spreadsheet_id, sheet, range, data, ctx)
